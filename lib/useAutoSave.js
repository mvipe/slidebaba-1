...
export function useAutoSave({ user, docId, setDocId, build, format = "slides", delay = 1500 }) {
  ...
  const doSave = useCallback(async (force = false) => {
    ...
    try {
      ...
      // While the document had no id yet, the local draft was written under the ":new" key.
      // Only the real-id key was being cleared, so that ":new" draft survived forever and
      // every fresh editor session popped an "unsaved changes on this device" prompt
      // offering an unrelated old document. Clear both.
      if (wasNew) clearLocalDraft(localKey(user.uid, null, format));
      return { ok: true, id };
    } catch (e) {
