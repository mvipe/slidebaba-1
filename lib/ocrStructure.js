...
function unitsFromMarkdown(markdown) {
  ...
  for (const line of lines) {
    ...
    if (md) {
      const inner = md[2].trim();
      // A "# प्रश्न 3" heading is a question, not a document title.
      if (detectMarker(inner).kind === "question") { buf.push(inner); continue; }
      flush();
  ...
  while (i < n) {
    ...
    while (p < run.length) {
      ...
      if (best && best.len >= 2) {
        ...
        if (shortEnough && !questionish && (hasStem || best.len >= 3 || bracketedAll)) {
...
function linesToSections(lines) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (isOption[i]) { ensure(); current.body.push(line.text); continue; }
    ...
    if (meta.kind === "label") {
      ...
      const label = meta.bracketed ? `(${meta.symbol})` : String(meta.symbol || "").trim();
      open(lastQuestionLabel ? `${lastQuestionLabel} ${label}` : label);
      if (meta.rest.trim()) current.body.push(meta.rest);
      continue;
    }
...
export function buildDocument(paddle, fallbackTitle = "Untitled Document") {
  ...
  // Rules fallback: if nothing split, keep paragraphs rather than one giant blob.
  if (sections.length <= 1) {
    const whole = sections[0]?.body || unmaskProtected(lines.map((l) => l.text).join("\n"), store);
    const paras = whole.split(/\n{2,}/).map((p) => p.trim()).filter(Boolean);
    if (paras.length > 1) sections = paras.map((p, i) => ({ heading: sections[0]?.heading && i === 0 ? sections[0].heading : "", body: p }));
  }

  let title = cleanHeading(unmaskProtected(picked.title || "", store));
  if (!title) {
    const idx = sections.findIndex((s) => s.body);
    const firstLine = (sections[idx]?.body || "").split("\n").find((l) => l.trim());
