 ...
 */
// The Urdu Nastaliq webface (Google Fonts). Urdu text is the SAME Unicode whether it is
// shown in Arabic-style Naskh or in flowing Nastaliq — only the font differs. We append
// this to every text stack so the browser's per-GLYPH fallback renders Arabic-script
// characters (Urdu) in Nastaliq while Latin / Devanagari keep the chosen family.
export const URDU_FONT = "Noto Nastaliq Urdu";

// Arabic block + Arabic Supplement + Extended-A + presentation forms = Urdu/Arabic/Persian.
const ARABIC_RE = /[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/g;

/** True when the text is mostly Arabic-script (Urdu) — worth Nastaliq line-spacing. */
export function isArabicHeavy(text) {
  const t = String(text || "");
  if (!t) return false;
  const arabic = (t.match(ARABIC_RE) || []).length;
  const letters = (t.match(/[\p{L}]/gu) || []).length || 1;
  return arabic >= 3 && arabic / letters > 0.4;
}

/**
 * Nastaliq needs far more vertical room than Latin or Naskh, or its tall diagonal
 * ligatures collide between lines. slideLayout measures with the same value.
 */
export const URDU_LINE_HEIGHT = 1.9;
...
export function fontStack(family) {
  ...
  // Nastaliq fallback is appended for every stack so Urdu always renders correctly,
  // even inside a slide whose chosen family is a Latin font.
  return generic ? `${f}, '${URDU_FONT}'` : `'${f}', '${URDU_FONT}', sans-serif`;
}
...
export function frameStyle(el) {
  ...
  return {
    ...
    // Only emit when it actually changes anything, so the exported CSS stays clean.
    ...(opacity < 1 ? { opacity } : {}),
  };
}

/** Element opacity is stored 0..1; anything missing/invalid means fully opaque. */
...
export function textStyle(el, theme) {
  ...
  const urdu = isArabicHeavy(el.content);
  return {
    display: "block",
    width: "100%",
    fontSize: `${el.fontSize || 20}px`,
    // Urdu gets Nastaliq first; other scripts keep the chosen family (with Nastaliq only
    // as a fallback for any stray Arabic glyph).
    fontFamily: urdu ? `'${URDU_FONT}', ${fontStack(el.fontFamily)}` : fontStack(el.fontFamily),
    fontWeight: el.bold ? 700 : 400,
    fontStyle: el.italic ? "italic" : "normal",
    textDecoration: deco,
    // Right-to-left for Urdu; honour an explicit non-default alignment, else right-align.
    ...(urdu ? { direction: "rtl" } : {}),
    textAlign: urdu ? (el.align && el.align !== "left" ? el.align : "right") : (el.align || "left"),
    color: el.color || theme?.text || "#0b0a18",
    background: el.highlight || "transparent",
    lineHeight: isArabicHeavy(el.content) ? URDU_LINE_HEIGHT : TEXT_LINE_HEIGHT,
    whiteSpace: "pre-wrap",
