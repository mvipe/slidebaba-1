...
import {
  DESIGN_W, DESIGN_H, styleToCss, frameStyle, textStyle, shapeInnerStyle, lineBarStyle,
  imageStyle, isTextElement, fontStack, TEXT_PAD_X, TEXT_PAD_Y, TEXT_LINE_HEIGHT,
  tableHTML, tableRows, tableColWidths,
...
async function elementHTML(el, theme) {
  ...
  if (type === "table") {
    // tableHTML() is the same builder the editor renders with, so the PDF cannot drift
    // from the canvas. The snapshot pipeline rasterises real DOM, so nothing else is needed.
    return `<div style="${frame}"><div class="kx" style="width:100%;height:100%;">` +
           `${tableHTML(el, theme, (cell) => renderMixed(String(cell || "")))}</div></div>`;
  }
...
export async function exportSlidesPptx(slides, THEMES, DEFAULT_THEME, title, onProgress) {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (const el of sl.elements || []) {
      ...
      const type = el.type || "text";
      // Editor opacity (0..1) -> PowerPoint transparency (0..100 % transparent).
      const alpha = Number.isFinite(Number(el.opacity)) ? Math.max(0, Math.min(1, Number(el.opacity))) : 1;
      const transp = alpha < 1 ? Math.round((1 - alpha) * 100) : 0;

      if (type === "image") {
        // The editor uses object-fit:fill, i.e. stretch to the frame — which is exactly
        // what addImage does without a `sizing` option.
        s.addImage({ data: await toDataUrl(el.src), ...box, ...(transp ? { transparency: transp } : {}) });
      } else if (type === "rect") {
        ...
        if (radius > 0) {
          ...
          s.addShape("roundRect", {
            ...
            fill: { color: pptColor(el.fill || "#6d3aed"), ...(transp ? { transparency: transp } : {}) },
            line,
          });
        } else {
          s.addShape("rect", { ...box, fill: { color: pptColor(el.fill || "#6d3aed"), ...(transp ? { transparency: transp } : {}) }, line });
        }
      } else if (type === "ellipse") {
        s.addShape("ellipse", {
          ...box,
          fill: { color: pptColor(el.fill || "#ec4899"), ...(transp ? { transparency: transp } : {}) },
        ...
        // A NATIVE PowerPoint table — editable in PowerPoint, not a flattened image.
        // Cell maths is converted to Unicode the same way body text is: PowerPoint has no
        // KaTeX, so "$x^2$" would otherwise export as literal dollar-sign markup.
        const rows = tableRows(el);
        const widths = tableColWidths(el);
        const useHeader = Boolean(el.header ?? true) && rows.length > 1;
        const bw = Number(el.borderWidth ?? 1) || 0;
        const border = bw
          ? { type: "solid", pt: Math.max(0.25, bw * PT), color: pptColor(el.borderColor || "#94a3b8") }
          : { type: "none" };
        const cellSizePt = Math.round((el.fontSize || 15) * PT * 2) / 2;

        const tableRowsPptx = rows.map((row, r) => row.map((cell) => {
          const isHead = useHeader && r === 0;
          return {
            text: mixedToUnicode(String(cell || "")),
            options: {
              bold: isHead && (el.headerBold ?? true),
              fill: isHead
                ? (isTransparent(el.headerFill) ? undefined : { color: pptColor(el.headerFill || "#eef2f7") })
                : (isTransparent(el.cellFill) ? undefined : { color: pptColor(el.cellFill) }),
            },
          };
        }));

        s.addTable(tableRowsPptx, {
          ...box,
          colW: widths.map((w) => (w / 100) * box.w),
          border,
          fontSize: cellSizePt,
          ...
          valign: "top",
          margin: [
            (el.cellPadY ?? 5) * PT, (el.cellPadX ?? 8) * PT,
            (el.cellPadY ?? 5) * PT, (el.cellPadX ?? 8) * PT,
          ],
          autoPage: false,
        });
        ...
        // Urdu: Nastaliq face + right-to-left so PowerPoint shows it correctly too.
        const urdu = /[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/.test(String(el.content || ""));
        s.addText(pptxTextRuns(el, theme), {
          ...box,
          fontSize: sizePt,
          fontFace: urdu ? "Noto Nastaliq Urdu" : (el.fontFamily || "Inter"),
          ...(urdu ? { rtlMode: true } : {}),
          bold: !!el.bold,
          italic: !!el.italic,
          underline: el.underline ? { style: "sng" } : undefined,
          strike: !!el.strike,
          align: urdu ? (el.align && el.align !== "left" ? el.align : "right") : (el.align || "left"),
          color: pptColor(el.color || `#${theme.pptText}`, theme.pptText),
