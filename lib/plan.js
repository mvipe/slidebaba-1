...
//
// SPECIAL ONE-TIME PLANS (quarter, triennial)
//   These are fixed-length promo plans, not part of the monthly/yearly toggle. They carry
//   a `durationDays` (90 / 1095) that cycleInfo() uses instead of the 30/365 the period
//   would imply, an `oneTime` flag so the UI can group them apart, and the SAME value in
//   both `m` and `y` for every field (there is one price and one allowance regardless of
//   which key planLimits() happens to read). Everything else in the pipeline —
//   planLimits, remaining, applyPlan, maybeRenew — keeps working unchanged.
export const PLANS = {
  ...
  // 3 months, ₹3,000 — sized so the marketing "10,000 slides" is reachable (~1,000 docs).
  quarter:   { label: "3-Month", m: 3000,  y: 3000,  credits: { m: 1000,  y: 1000 },  docs: { m: 1000,  y: 1000 },  slides: { m: "10,000",   y: "10,000" },     maxSlidesPerDoc: { m: 1800, y: 1800 }, maxPages: { m: 150, y: 150 }, pdf: true, ppt: true,  branding: true, teaching: true,  durationDays: 90,   oneTime: true },
  // 3 years, ₹20,000 — the most generous tier, everything on, valid for the full 3 years.
  triennial: { label: "3-Year",  m: 20000, y: 20000, credits: { m: 30000, y: 30000 }, docs: { m: 30000, y: 30000 }, slides: { m: "3,60,000+", y: "3,60,000+" }, maxSlidesPerDoc: { m: 2400, y: 2400 }, maxPages: { m: 200, y: 200 }, pdf: true, ppt: true,  branding: true, teaching: true,  durationDays: 1095, oneTime: true },
};
export const PLAN_ORDER = ["free", "basic", "medium", "high", "quarter", "triennial"];
export const RANK = { free: 0, basic: 1, medium: 2, high: 3, quarter: 4, triennial: 5 };
...
export function cycleInfo(profile) {
  ...
  // A one-time promo plan (quarter/triennial) is valid for its own fixed span; everything
  // else follows the monthly/yearly period.
  const days = PLANS[plan]?.durationDays ?? (period === "yearly" ? 365 : 30);
