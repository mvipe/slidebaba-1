...
import { isArabicHeavy, URDU_LINE_HEIGHT, URDU_FONT } from "@/lib/slideRender";
...
function measureHeight(text, widthPx, fontSize, { bold = false, fontFamily = "Inter" } = {}) {
  // Urdu is measured with the SAME Nastaliq font + line-height it renders with, so a
  // fitted Urdu slide never overflows or leaves a huge gap.
  const urdu = isArabicHeavy(text);
  const lh = urdu ? URDU_LINE_HEIGHT : 1.35;
  const fam = urdu ? `${fontFamily},'${URDU_FONT}',sans-serif` : `${fontFamily},'${URDU_FONT}',sans-serif`;
  const d = document.createElement("div");
  d.className = "kx";
  d.style.cssText =
    `position:absolute;left:-99999px;top:0;visibility:hidden;width:${widthPx}px;` +
    `font-size:${fontSize}px;font-family:${fam};font-weight:${bold ? 700 : 400};` +
    `line-height:${lh};white-space:pre-wrap;padding:2px 6px;box-sizing:border-box;`;
