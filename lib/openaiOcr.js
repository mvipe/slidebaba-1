 ...
 *   OPENAI_MAX_TOKENS         default: 8000            (per vision call)
 *   OPENAI_TIMEOUT_MS         default: 90000
 *   OPENAI_MAX_WAIT_MS        default: 150000
 *   OPENAI_CONCURRENCY        default: 2
...
export function getOpenAIConfig() {
  ...
  return {
    ...
    maxTokens: num("OPENAI_MAX_TOKENS", 8000, 1000),
    timeoutMs,
    maxWaitMs: Math.max(timeoutMs, num("OPENAI_MAX_WAIT_MS", 150000, 20000)),
    concurrency: num("OPENAI_CONCURRENCY", 2, 1),
...
- Marathi is written in Devanagari — the Devanagari rules above apply to it in full.
- The same rules apply to every other non-Latin script on the page: Urdu (Arabic script), Bengali, Tamil, Telugu, Gujarati, Punjabi, Odia, Kannada, Malayalam, Arabic.
...
- Each question starts on its own line, beginning with its printed label exactly as printed ("प्रश्न 3.", "Q7", "12.", "(iv)").
...
export async function openaiOcr(image, cfgIn, opts = {}) {
  ...
  return withGate(cfg.concurrency, async () => {
    ...
    // completion unless there is comfortably time for it to finish.
