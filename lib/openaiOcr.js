...
import { readImageInput } from "@/lib/paddleocr";
...
function looksTransliterated(text) {
  const dev = countMatches(text, DEVANAGARI_RE);
  if (dev > 20) return false;                 // real Devanagari is present — nothing to fix
  return countMatches(text, ROMANISED_HINDI_RE) >= 3;
}
...
export async function openaiOcr(image, cfgIn, opts = {}) {
  ...
  return withGate(cfg.concurrency, async () => {
    ...
    if (r.ok && looksTransliterated(markdown) && deadline - Date.now() > 12000) {
