...
/**
 * All the text a deck prints, from every element type that holds text.
 *
 * Tables are the reason this exists as a helper. The three harvesting passes below used to
 * test `isTextElement(el)` and skip everything else, so a table's cell text contributed no
 * font families and no codepoints — the PDF then embedded a subset missing exactly the
 * glyphs the table needed, and rendered them as tofu boxes. Devanagari made that instant
 * and total rather than subtle.
 */
function elementTexts(el) {
  if (isTextElement(el)) return [String(el.content || "")];
  if (el?.type === "table") {
    const out = [];
    for (const row of Array.isArray(el.rows) ? el.rows : []) {
      for (const cell of Array.isArray(row) ? row : []) out.push(String(cell ?? ""));
    }
    return out;
  }
  return [];
...
function deckFamilies(slides, needsMath) {
  ...
  // Urdu renders via a Nastaliq FALLBACK font, so it never appears as an element's
  // fontFamily — force it in whenever the deck actually contains Arabic-script text,
  // or the embedded subset would miss those glyphs and the PDF would show tofu boxes.
  const hasArabic = slides.some((sl) => (sl.elements || []).some((el) =>
    elementTexts(el).some((t) => /[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/.test(t))));
  if (hasArabic) set.add("noto nastaliq urdu");
    ...
    for (const el of sl.elements || []) {
      ...
      if (texts.length) {
        ...
        for (const t of texts) {
          let m;
          re.lastIndex = 0;
          while ((m = re.exec(t))) set.add(m[1].trim().replace(/^["']|["']$/g, "").toLowerCase());
        }
      }
    }
  }
...
export function deckNeedsMath(slides) {
  return slides.some((sl) =>
    (sl.elements || []).some((el) => elementTexts(el).some((t) => /[$\\^_]/.test(t))));
}
