...
export const DEFAULT_DESIGN = {
  ...
  bgOpacity: 1,      // 0..1 — fades the background IMAGE (watermark), text stays solid
...
export function pageStyle(design) {
  ...
  // A background image is drawn by pageBgLayerStyle() so its opacity is independent of
  // the note text. Here we just lay down a solid colour beneath it.
  if (d.pageBgImage) {
    return { ...base, background: d.pageBg || "#ffffff" };
  }
  if (t.grid && !d.pageBg) {
...
/** 0..1, anything invalid means fully opaque. */
export function clampBgOpacity(v) {
  const n = Number(v);
...
/**
 * Style for the separate background-image layer, or null when there is no image.
 * It sits behind the note content so its opacity fades ONLY the image — the text
 * above it stays fully readable, which is what a watermark actually needs.
 */
export function pageBgLayerStyle(design) {
  const d = normalizeDesign(design);
  if (!d.pageBgImage) return null;
  return {
    position: "absolute",
    inset: 0,
    ...
    opacity: clampBgOpacity(d.bgOpacity),
    zIndex: 0,
    pointerEvents: "none",
  };
}
