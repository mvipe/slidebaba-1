...
import { writeDocument, readPayload, deleteDocumentDeep, withTimeout, storedFormats } from "@/lib/docStore";

export { storedFormats };
 ...
 *
 * `header` is in this list because the A4 notes header carries a custom logo as a base64
 * data URL. It used to be written to the parent document, which Firestore caps at 1 MiB —
 * so a logo over ~750 KB made EVERY save of that document fail, permanently, with an
 * opaque "invalid-argument". Routing it into the chunked payload removes the cap.
 * Reads are unaffected: loadDocument() spreads the payload over the metadata, so callers
 * still just read `doc.header`. */
const HEAVY_KEYS = ["slides", "notesHtml", "items", "header"];
...
export async function listDocuments(uid) {
  ...
  // THROWS on failure, deliberately. Returning [] on an error rendered a permission or
  // network failure as the empty state — "No documents here yet" — which reads as data
  // loss to someone whose documents are all still there. Callers must show the error.
...
export async function loadDocument(uid, id, format) {
  ...
  // Say so plainly when this document has nothing stored in the requested format.
  // Previously this returned an empty payload, the editor quietly ignored the handoff,
  // and the first keystroke created a SECOND document — orphaning the original. That is
  // the "my old files won't open" report, and the fork is why work appeared to vanish.
  const has = storedFormats(data);
  if (!payload && !has[fmt === "notes" ? "notes" : "slides"]) {
    const other = fmt === "notes" ? "slides" : "notes";
    const err = new Error(
      has[other]
        ? `This document has no ${fmt === "notes" ? "A4 notes" : "slides"} version saved — open it as ${other === "notes" ? "A4" : "PPT"} instead.`
        : "This document has no saved content yet."
    );
    err.code = "empty-format";
    err.availableFormat = has[other] ? other : null;
    throw err;
  }
  ...
  return {
    id, ...data, ...(payload || {}),
    formats: has,
    chunkCount: data[`chunks_${fmt === "notes" ? "notes" : "slides"}`] ?? null,
  };
}
